...
export async function getDomicilios() {
    const ENDPOINT = 'http://localhost:8080/domicilio/all';
...
export async function getDomicilioPorId(id: number) {
    const ENDPOINT = `http://localhost:8080/domicilio/${id}`;
...
export async function saveDomicilio(domicilio?: Domicilio) {
    let endpoint = 'http://localhost:8080/domicilio';
    let method: string = "POST";

    if (domicilio && domicilio.id !== 0) {
        endpoint = `http://localhost:8080/domicilio/${domicilio.id}`;
        method = "PUT";
    }
